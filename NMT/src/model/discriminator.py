...
class Discriminator(nn.Module):
	...
	def __init__(self, params):
		...
		if self.dis_layers == 0:
			input_dim = self.input_dim
			input_dim *= (2 if params.attention and not params.dis_input_proj else 1)
		else:
			input_dim = self.dis_hidden_dim
		self.out_language = nn.Linear(input_dim, self.n_langs)
		self.out_auxillary = nn.Linear(input_dim, 2)

	def forward(self, input):
		return self.out_language(self.layers(input))

	def forward_aux(self, input):
		return self.out_auxillary(self.layers(input))
