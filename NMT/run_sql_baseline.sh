...
if [ $ANNOT_UPPCT -eq 0 -a $MINED_UPPCT -eq 0 ]; then
	python3 main.py --exp_name $CONFIG --exp_id "results_baseline_${BATCH_SIZE}_${USE_AUX}${BPE}${SEQ_EXT}${WGAN_EXT}" --dump_path $BASE_DIR --transformer True --n_enc_layers 4 --n_dec_layers 4 --share_enc 3 --share_dec 3 --share_lang_emb True --share_output_emb True --langs 'x,y' --n_mono -1  --mono_dataset "x:${BASE_DIR}/${CONFIG}/data/para.x${BPE}.full.pth,,;y:${BASE_DIR}/${CONFIG}/data/para.y${BPE}.full.pth,," --n_para -1 --para_dataset "x-y:${BASE_DIR}/${CONFIG}/data/para.XX${BPE}.full.pth,${BASE_DIR}/${CONFIG}/data/conala-dev.XX${BPE}.full.pth,${BASE_DIR}/${CONFIG}/data/conala-test.XX${BPE}.full.pth" --para_directions 'x-y,y-x' --pivo_directions 'x-y-x,y-x-y' --pretrained_emb "${BASE_DIR}/${CONFIG}/data/concat${BPE}.full.vec"  --pretrained_out True --lambda_xe_para '0:1,100000:0.1,300000:0'  --lambda_xe_otfd 1 --lambda_lm 0 --lambda_dis 1.0 --n_dis 1 --dis_hidden_dim 1024 --dis_smooth 0.1 --otf_num_processes 10 --otf_sync_params_every 1000 --enc_optimizer adam,lr=0.00001 --epoch_size -1 --stopping_criterion bleu_x_y_valid,25 --cudnn_enabled $USE_CUDNN --batch_size ${BATCH_SIZE}  --max_epoch $EPOCH --dis_aux 0 --seq_dis $USE_SEQ --lambda_seq_dis 1.0 --wgan $USE_WGAN
else
